...
-(id) init
{
	if( (self=[super init]))
    {
        ...
        [[[CCDirector sharedDirector] openGLView] addSubview:camPreview];
        camPreview.backgroundColor = [UIColor greenColor];
        camPreview.frame = CGRectMake(120, 0, 75, 75);
...
- (void) addBulletTypeTip
{
    ...
    if (propTime > 1500)
    {
        ...
        [prop initWithType:arc4random()%2 ? propsTypeBomb : propsTypeBullet];
    ...
    if (!isGameOver)
    {
        ...
        if(CVPixelBufferLockBaseAddress(imageBuffer, 0) == kCVReturnSuccess)
        {
            ...
            switch (pixelFormat)
            {
                ...
                {
                    miniEvent event;
                    int ret = avatar_.OnTrack(baseAddress, img_size,
                                                        img_width,img_height,
                                                        img_widstp, (char*)&event);
                    
                    if(ret == 0)
                        dispatch_async(dispatch_get_main_queue(), ^{
                            float x = (uint16Tofloat_t(event.head_rx) - 0.5) * PI;
                            float y = (uint16Tofloat_t(event.head_ry) - 0.5) * PI;
                            CGPoint point = player.position;
                            [self boundingBox];
                            point.x -= y * 40;
                            point.y += x * 40;
                            printf("box: %f, %f, %f, %f\n", [self boundingBox].size.width,
                                   [self boundingBox].size.height, point.x, point.y);
                            if (point.x < 0)
                                point.x = 0;
                            else if (point.x > [self boundingBox].size.width)
                                point.x = [self boundingBox].size.width;
                            if (point.y < 0)
                                point.y = 0;
                            else if (point.y > [self boundingBox].size.height)
                                point.y = [self boundingBox].size.height;
                            player.position = point;
                            if (ucharTofloat(event.blendshape[12]) > 0.3) {
                                for (int i = 0; i < 10; ++i)
                                    [self firingBullets];
                            }
                        });
